...
void record(int index, char ch) {
    nodes[index].count[ch - 'a']++;
}
char pop(int index) {
    node_t *node = &nodes[index];
    for (; !node->count[node->cur]; node->cur++) {}
    node->count[node->cur]--;
    return 'a' + node->cur;
}
char *smallestStringWithSwaps(char *s, int **pairs, int size, int *colSize) {
    for (int i = 0; s[i]; ++i) {
        ancestor[i] = i, nodes[i] = (node_t) { 0 };
    }
