...
// 如果为 true 就意味着你应该只包括veganFriendlyi 为 true 的餐馆，为 false 则意味着可以包括任何餐馆。
...
static int cmp(const void *a, const void *b) {
    ...
    if (pa[1] == pb[1]) return pb[0] - pa[0];
    return pb[1] - pa[1];
}
int *filterRestaurants(int **restaurants, int size, int *colSize, int veganFriendly, int maxPrice, int maxDistance,
                       int *returnSize) {
    int help[size], len = 0;
    qsort(restaurants, size, sizeof(int *), cmp);
    for (int i = 0; i < size; ++i) {
        if (veganFriendly && !restaurants[i][2]) continue;
        if (restaurants[i][3] > maxPrice) continue;
        if (restaurants[i][4] > maxDistance) continue;
        help[len++] = restaurants[i][0];
    }
