...
//
//
// Related Topics 记忆化搜索 数组 动态规划 👍 85 👎 0

long long sellingWood(int m, int n, int **prices, int size, int *colSize) {
    long long dp[m][n];
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            for (int h = 1; h + h <= i + 1; ++h) {
                dp[i][j] = fmax(dp[i][j], dp[h - 1][j] + dp[i - h][j]);
            }
            for (int w = 1; w + w <= j + 1; ++w) {
                dp[i][j] = fmax(dp[i][j], dp[i][w - 1] + dp[i][j - w]);
            }
        }
    }
    return dp[m - 1][n - 1];
