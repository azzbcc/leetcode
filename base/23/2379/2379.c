...
// 输出：0
// 解释：
// 不需要任何操作，因为已经有 2 个连续的黑块。所以我们返回 0 。
//
//
//
//
...
//
//
// Related Topics 字符串 滑动窗口 👍 44 👎 0

int minimumRecolors(char *blocks, int k) {
    int ans = 0;
    for (int i = 0; i < k; ans += blocks[i++] == 'W') {}
    for (int i = k, tmp = ans; blocks[i]; ++i) {
        tmp += blocks[i] == 'W', tmp -= blocks[i - k] == 'W';
        ans = fmin(ans, tmp);
    }
    return ans;
}
