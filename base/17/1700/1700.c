// 学校的自助午餐提供圆形和方形的三明治，分别用数字 0 和 1
...
//
//
// Related Topics 栈 队列 数组 模拟 👍 89 👎 0

int countStudents(int *students, int studentsSize, int *sandwiches, int sandwichesSize) {
    int s[2] = { 0 };
    for (int i = 0; i < studentsSize; s[students[i++]]++) {}
    for (int i = 0; i < sandwichesSize && s[sandwiches[i]]; s[sandwiches[i++]]--) {}
    return s[0] + s[1];
}
