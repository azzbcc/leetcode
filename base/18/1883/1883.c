...
node_t next(node_t now, int dist, int speed, bool skip) {
    if (skip) {
        now.left += dist;
    } else {
        now.cnt += now.left > 0, now.left = dist;
    }
    now.cnt += now.left / speed, now.left %= speed;
    return now;
}
node_t min(node_t a, node_t b) {
    if (a.cnt < b.cnt) return a;
    if (a.cnt > b.cnt) return b;
    if (a.left < b.left) return a;
    return b;
}
int minSkips(int *dist, int size, int speed, int before) {
    node_t dp[size];
    for (int i = 0; i < size; ++i) {
        dp[i] = (node_t) { dist[0] / speed, dist[0] % speed };
    }
    for (int i = 1; i < size; ++i) {
        for (int j = size - 1; j > 0; --j) {
            dp[j] = min(next(dp[j], dist[i], speed, false), next(dp[j - 1], dist[i], speed, true));
        }
        dp[0] = next(dp[0], dist[i], speed, false);
    }
    for (int i = 0; i < size; ++i) {
        if (dp[i].cnt + (dp[i].left > 0) <= before) return i;
    }
    return -1;
}