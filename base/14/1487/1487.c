...
char **getFolderNames(char **names, int size, int *returnSize) {
    char str[21];
    hash_t hash = NULL, cur, next;
    char **ans  = malloc((*returnSize = size) * sizeof(char *));
    for (int i = 0; i < size; ++i) {
        sprintf(str, "%s", names[i]);
        HASH_FIND_STR(hash, str, cur);
        for (hash_t tmp; cur;) {
            sprintf(str, "%s(%d)", names[i], cur->next++);
            HASH_FIND_STR(hash, str, tmp);
            if (!tmp) break;
        }
        cur = malloc(sizeof(*cur)), cur->next = 1, cur->str = ans[i] = strdup(str);
