...
int **floodFill(int **image, int imageSize, int *imageColSize, int sr, int sc, int newColor, int *returnSize,
    ...
    *returnColumnSizes = calloc(imageSize, sizeof(int));
    ...
    int front = 0, rear = 1;
    point_t queue[0x1000] = { { sr, sc } };
    while (front < rear) {
        point_t now       = queue[front++];
        ans[now.x][now.y] = newColor;
        ...
        for (int i = 0; i < sizeof(wards) / sizeof(wards[0]); ++i) {
            ...
            if (next.x < 0 || next.x >= imageSize) continue;
            if (next.y < 0 || next.y >= *imageColSize) continue;
            if (ans[next.x][next.y] != oldColor) continue;
