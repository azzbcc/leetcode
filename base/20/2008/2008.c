...
typedef struct __std_node node_t;
struct __std_node {
    int start, weight;
    node_t *next;
};
long long maxTaxiEarnings(int n, int **rides, int size, int *colSize) {
    long long dp[n + 1];
    node_t nodes[size], *map[n + 1];
    memset(dp, 0, sizeof(dp));
    memset(map, 0, sizeof(map));
    for (int i = 0; i < size; ++i) {
        int start = rides[i][0], end = rides[i][1], tip = rides[i][2];
        nodes[i] = (node_t) { start, end - start + tip, map[end] };
        map[end] = &nodes[i];
    }
    for (int i = 2; i <= n; ++i) {
        long long now = dp[i - 1];
        for (node_t *l = map[i]; l; l = l->next) {
            now = fmax(now, dp[l->start] + l->weight);
        }
        dp[i] = now;
    }
    return dp[n];
