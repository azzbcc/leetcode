...
int **allCellsDistOrder(int R, int C, int r0, int c0, int *returnSize, int **returnColumnSizes) {
    ...
    for (int r = 0; r < R; ++r) {
        for (int c = 0; c < C; ++c) {
            nodes[r * C + c] = (node_t) { r, c, abs(r - r0) + abs(c - c0) };
        }
    }
    qsort(nodes, R * C, sizeof(node_t), cmp);

    ans = malloc((*returnSize = R * C) * sizeof(int *)), *returnColumnSizes = malloc(R * C * sizeof(int));
    for (int i = 0; i < R * C; ++i) {
        (*returnColumnSizes)[i] = 2;
        ans[i] = malloc(2 * sizeof(int)), ans[i][0] = nodes[i].r, ans[i][1] = nodes[i].c;
    }
