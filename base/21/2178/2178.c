...
//
//
// Related Topics 贪心 数学 回溯 👍 63 👎 0

long long *maximumEvenSplit(long long sum, int *returnSize) {
    if (sum % 2) return malloc((*returnSize = 0));
    int len        = (sqrtl(sum * 4 + 1) - 1) / 2;
    long long *ans = malloc((*returnSize = len) * sizeof(long long));
    for (int i = 1; i < len; sum -= ans[i - 1] = i * 2, ++i) {}
    ans[len - 1] = sum;
    return ans;
}
