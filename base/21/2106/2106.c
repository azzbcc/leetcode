...
//
//
// Related Topics 数组 二分查找 前缀和 滑动窗口 👍 89 👎 0

int count(int **fruits, int start, int l, int r) {
    if (fruits[r][0] <= start) return start - fruits[l][0];
    if (fruits[l][0] >= start) return fruits[r][0] - start;
    return fruits[r][0] - fruits[l][0] + fmin(abs(start - fruits[l][0]), abs(start - fruits[r][0]));
}
int maxTotalFruits(int **fruits, int size, int *colSize, int startPos, int k) {
    int ans = 0;
    for (int l = 0, r = 0, sum = 0; r < size; ++r) {
        for (sum += fruits[r][1]; l <= r && count(fruits, startPos, l, r) > k; sum -= fruits[l++][1]) {}
        ans = fmax(ans, sum);
    }
    return ans;
}
