...
// security[i - time] >= security[i - time + 1] >= ... >= security[i] <= ... <= security[i + time - 1] <= security[i +
// time].
//
...
int *goodDaysToRobBank(int *security, int size, int time, int *returnSize) {
    bool flag[size];
    int len = 0, help[size];
    memset(flag, true, sizeof(flag));

    for (int i = 0, l = 0; i < size; ++i) {
        if (i && security[i] <= security[i - 1]) {
            l++;
        } else {
            l = 0;
        }
        flag[i] = flag[i] && l >= time;
    }
    for (int i = size - 1, r = 0; i >= 0; --i) {
        if (i + 1 < size && security[i] <= security[i + 1]) {
            r++;
        } else {
            r = 0;
        }
        flag[i] = flag[i] && r >= time;
    }
