...
static int cmp(const void *a, const void *b) {
    ...
    return pb->grow - pa->grow;
}
int earliestFullBloom(int *plantTime, int size, int *growTime, int growTimeSize) {
    int ans = 0;
    node_t nodes[size];

    for (int i = 0; i < size; ++i) {
        nodes[i] = (node_t) { plantTime[i], growTime[i] };
    }
    qsort(nodes, size, sizeof(node_t), cmp);
    for (int i = 0, plant = 0; i < size; ++i) {
        plant += nodes[i].plant;
        ans = fmax(ans, plant + nodes[i].grow);
    }
    return ans;
}