...
//
//
// Related Topics 数组 哈希表 👍 25 👎 0

#define N 50
int *distinctDifferenceArray(int *nums, int size, int *returnSize) {
    int suffix[size + 1], nc[2][N + 1] = { 0 };
    int *ans = malloc((*returnSize = size) * sizeof(int));

    suffix[size] = 0;
    for (int i = size - 1; i >= 0; --i) {
        suffix[i] = suffix[i + 1] + !nc[0][nums[i]]++;
    }
    for (int i = 0, prefix = 0; i < size; ++i) {
        prefix += !nc[1][nums[i]]++;
