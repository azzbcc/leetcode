...
// 输出: 0.5
// 解释:
// 我们在顶层（下标是（0，0）倒了两杯香槟后，有一杯量的香槟将从顶层溢出，位于（1，0）的玻璃杯和（1，1）的玻璃杯平分了这一杯香槟，所以每个玻璃杯有
// 一半的香槟。
//
//
// 注意:
...
double champagneTower(int poured, int query_row, int query_glass) {
    double help[101][100] = { poured };
    for (int i = 0; i <= query_row; ++i) {
        for (int j = 0; j <= i; ++j) {
            if (help[i][j] > 1) {
                help[i + 1][j] += help[i][j] / 2 - 0.5;
                help[i + 1][j + 1] += help[i][j] / 2 - 0.5;
                help[i][j] = 1.0;
            }
        }
    }
