...
bool validIPv4(char *ip) {
    for (int i = 0, c, sum; i < 4; ++i) {
        for (c = 0, sum = 0; isdigit(*ip) && c < 3; sum *= 10, sum += *ip++ - '0', ++c) {}
        if (!c || c == 2 && sum < 10 || c == 3 && (sum < 100 || sum > 255)) return false;
        if (i < 3 && *ip++ != '.') return false;
    }
    return *ip == '\0';
}
bool validIPv6(char *ip) {
    for (int i = 0, c; i < 8; ++i) {
        for (c = 0; isdigit(*ip) || *ip >= 'a' && *ip <= 'f' || *ip >= 'A' && *ip <= 'F'; ++ip, ++c) {}
        if (c == 0 || c > 4) return false;
        if (i < 7 && *ip++ != ':') return false;
    }
    return *ip == '\0';
}
char *validIPAddress(char *queryIP) {
    bool v4 = validIPv4(queryIP), v6 = !v4 && validIPv6(queryIP);
    return !v4 && !v6 ? "Neither" : (char *[]) { "IPv4", "IPv6" }[v6];
}
