...
int *numMovesStonesII(int *stones, int size, int *returnSize) {
    int *ans = calloc(*returnSize = 2, sizeof(int));
    qsort(stones, size, sizeof(int), cmp);
    if (stones[size - 1] - stones[0] == size - 1) return ans;

    ans[0] = size, ans[1] = fmax(stones[size - 2] - stones[0], stones[size - 1] - stones[1]) + 2 - size;
    for (int i = 0, j = 0; i < size; ++i) {
        for (; j + 1 < size && stones[j + 1] - stones[i] + 1 <= size; ++j) {}
        if (j - i + 1 == size - 1 && stones[j] - stones[i] + 1 == size - 1) {
            ans[0] = fmin(ans[0], 2);
        } else {
            ans[0] = fmin(ans[0], size - (j - i + 1));
        }
    }
    return ans;
}
