...
int getWinner(int *arr, int size, int k) {
    list_t list[size], *head = list, *tail = &list[size - 1], *tmp;
    if (k > size) k = size;
    for (int i = 0; i < size; ++i) {
        list[i] = (list_t) { arr[i], &list[(i + 1) % size] };
    }
    for (int c = 0; c < k; ++c) {
        if (head->data > head->next->data) {
            tmp = head->next, head->next = tmp->next, tail->next = tmp, tmp->next = head, tail = tmp;
        } else {
            c = 0, tail = head, head = head->next;
        }
    }
    return head->data;
}
