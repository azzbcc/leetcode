...
//
//
// Related Topics 贪心 数组 矩阵 👍 114 👎 0

int **restoreMatrix(int *rowSum, int rowSumSize, int *colSum, int colSumSize, int *returnSize,
                    int **returnColumnSizes) {
    int **ans;
    ans = malloc((*returnSize = rowSumSize) * sizeof(int *)), *returnColumnSizes = malloc(rowSumSize * sizeof(int));
    for (int i = 0; i < rowSumSize; ++i) {
        ans[i] = malloc(((*returnColumnSizes)[i] = colSumSize) * sizeof(int));
        for (int j = 0; j < colSumSize; ++j) {
            ans[i][j] = fmin(colSum[j], rowSum[i]), colSum[j] -= ans[i][j], rowSum[i] -= ans[i][j];
        }
    }
    return ans;
}