...
//
//
// Related Topics 数组 数学 动态规划 博弈 👍 93 👎 0

int stoneGameVII(int *stones, int size) {
    int dp[size + 1];

    memset(dp, 0, sizeof dp);
    for (int len = 0, alice = size & 1; len < size; ++len, alice ^= 1) {
        for (int i = 0; i + len < size; ++i) {
            if (alice) {
                dp[i] = fmax(dp[i], dp[i + 1]);
            } else {
                dp[i] = fmin(stones[i + len] + dp[i], stones[i] + dp[i + 1]);
            }
        }
    }
    return dp[0];
