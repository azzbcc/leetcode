...
//
//
// Related Topics 数组 双指针 字符串 👍 104 👎 0

bool check(char *s, char *w) {
    for (int cs, cw; *s || *w; s += cs, w += cw) {
        if (*s != *w) return false;
        for (cs = 1; *s == *(s + cs); ++cs) {}
        for (cw = 1; *w == *(w + cw); ++cw) {}
        if (cs < cw || cs > cw && cs < 3) return false;
    }
    return true;
}
int expressiveWords(char *s, char **words, int size) {
    int ans = 0;
    for (int i = 0; i < size; ans += check(s, words[i++])) {}
    return ans;
}