...
int profitableSchemes(int n, int minProfit, int *group, int size, int *profit, int pSize) {
    int dp[n + 1][minProfit + 1];

    memset(dp, 0, sizeof(dp));
    for (int i = 0; i <= n; dp[i++][0] = 1) {}
    for (int i = 1; i <= size; ++i) {
        int cost = group[i - 1], weight = profit[i - 1];
        for (int j = n; j >= cost; --j) {
            for (int k = minProfit; k >= 0; --k) {
                dp[j][k] = (dp[j][k] + dp[j - cost][max(0, k - weight)]) % MOD;
            }
        }
    }
