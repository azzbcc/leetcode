// 有一组 n 个人作为实验对象，从 0 到 n - 1编号，其中每个人都有不同数目的钱，以及不同程度的安静值（quietness）。
// 为了方便起见，我们将编号 为 x 的人简称为 "personx "。
//
...
//
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 110 👎 0

#define MAXN 500
#define MAXM (MAXN * (MAXN - 1) / 2)
int idx, h[MAXN], e[MAXM], ne[MAXM];
void edge_record(int a, int b) {
    e[idx] = b, ne[idx] = h[a], h[a] = idx++;
}
void dfs(int cur, int quiet[], int *ans) {
    if (ans[cur] != -1) return;
    ans[cur] = cur;
    for (int i = h[cur]; i >= 0; i = ne[i]) {
        dfs(e[i], quiet, ans);
        if (quiet[ans[cur]] > quiet[ans[e[i]]]) ans[cur] = ans[e[i]];
    }
}
int *loudAndRich(int **richer, int richerSize, int *colSize, int *quiet, int size, int *returnSize) {
    int *ans = malloc((*returnSize = size) * sizeof(int));

    memset(ans, -1, size * sizeof(int));
    idx = 0, memset(h, -1, size * sizeof(int));
    for (int i = 0; i < richerSize; ++i) {
        edge_record(richer[i][1], richer[i][0]);
    }
